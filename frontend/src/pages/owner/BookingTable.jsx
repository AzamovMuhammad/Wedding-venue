...
function BookingTable() {
  ...
  const fetchBookings = async () => {
    try {
      const res = await axios.get("http://localhost:4001/bookings", {
  ...
  const handleCancel = async (id) => {
    ...
    try {
      await axios.patch(
        `http://localhost:4001/bookings/${id}/cancel`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 bg-white rounded shadow mt-6">
        ...
        <tbody>
          {bookings.map((b) => (
            <tr key={b.id} className="hover:bg-pink-50">
              ...
              <td className="border px-4 py-2 text-center">{b.guest_count}</td>
              <td className="border px-4 py-2">
                {b.firstname && b.lastname
                  ? `${b.firstname} ${b.lastname} (${b.phone_number || "Noma’lum"})`
                  : "Foydalanuvchi"}
              </td>
              <td className="border px-4 py-2 text-center">{formatStatus(b.status)}</td>
              <td className="border px-4 py-2 text-center">
