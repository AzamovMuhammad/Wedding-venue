...
function VenueInfo() {
  ...
  useEffect(() => {
    async function fetchVenue() {
      ...
      try {
        const res = await axios.get(`http://localhost:4001/venue/${id}`, { // Port va yo'lni tekshiring
          headers: { Authorization: `Bearer ${token}` },
        });
        setVenue(res.data.venue);
      } catch (err) {
        setError(err.response?.data?.message || "To'yxona ma'lumotlarini yuklashda xatolik yuz berdi");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      setBookingError(err.response?.data?.message || "Bron qilishda server xatosi yuz berdi.");
    } finally {
      setIsBooking(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg mt-10 mb-10">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div>
          <p className="mb-2"><strong>Rayon:</strong> {venue.district_name || "Noma'lum"}</p>
          <p className="mb-2"><strong>Manzil:</strong> {venue.address}</p>
          <p className="mb-2"><strong>Sig‘im:</strong> {venue.capacity} kishi</p>
          <p className="mb-2"><strong>Narx (1 o‘rindiq):</strong> {venue.price_per_seat ? `${venue.price_per_seat.toLocaleString()} so‘m` : "Noma'lum"}</p>
        </div>
        <div>
          <p className="mb-2"><strong>Telefon:</strong> {venue.phone_number || "Ko‘rsatilmagan"}</p>
          <p className="mb-2"><strong>Egasi:</strong> {venue.owner_firstname || ""} {venue.owner_lastname || "Noma'lum"}</p>
          <p className="mb-2"><strong>Status:</strong> <span className={`px-2 py-1 text-sm rounded-full ${venue.status === 'tasdiqlangan' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{venue.status || "Noma'lum"}</span></p>
        </div>
      </div>

      <div className="mb-8">
        <h3 className="text-2xl font-semibold mb-4 text-gray-800">Suratlar</h3>
        {venue.images && venue.images.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {venue.images.map((imgUrl, idx) => (
              <img
                key={idx}
                src={imgUrl.startsWith('http') ? imgUrl : `http://localhost:4001${imgUrl}`} // Agar URL to'liq bo'lmasa, localhost qo'shish
      ...
      <div className="mt-10 pt-6 border-t border-gray-200">
        <h3 className="text-2xl font-semibold mb-5 text-gray-800">To'yxonani Bron Qilish</h3>
        {bookingMessage && <p className="text-green-600 mb-3 p-3 bg-green-50 rounded-md">{bookingMessage}</p>}
        {bookingError && <p className="text-red-600 mb-3 p-3 bg-red-50 rounded-md">{bookingError}</p>}
