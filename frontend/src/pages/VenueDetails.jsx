...
function VenueDetails() {
  ...
  const handleDelete = async () => {
    if (!window.confirm("To’yxonani o‘chirishingizni tasdiqlaysizmi?")) return;

    try {
      await axios.delete(`http://localhost:4001/venue/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setMessage("To’yxona muvaffaqiyatli o‘chirildi");
      setTimeout(() => navigate("/"), 1500); // 1.5 soniyadan keyin bosh sahifaga yo‘naltirish
    } catch (err) {
